...
| 영역 | 주 담당 | 상태 |
|---|---|---|
| `server/` | 장민석 | 스포크 5종 규칙·`ai/` 합산(`masking`·`closure_gate`·**`postcall`**·`retrieval`·`trigger`·`call_guard` + S3 있으면 `uploads`). 테스트 server 696 + integration 12 · ai 225 · 계약 4·3종 KEPT (09-15 실측). **09-15**: 통화 후 초안 501 걷음 — **규칙 발췌**(`decisions/306`, 유형은 null, 품질 측정 불가, 저장은 `w7-postcall-persistence`) · **상담원 전용 토큰**(`decisions/307`) — `agent_token`(해시만) · `/admin/agent-tokens` · `POST /hub/blacklist-requests` 는 토큰 필수·`requested_by` 본문에서 제거. 스키마 26 테이블 — 운영 `agent_token` 마이그레이션은 **정성윤 님이 적용했다고 보고(09-15), 출력은 못 봤다** · 서버 이미지 태그 `0.1.8` 로 올림(PR #86, 머지 전). 고객 연결(발신 번호 → HMAC → `GET /hub/calls?customer_id=`)은 로컬 실DB 로 끝까지 확인. `POST /hub/calls`(통화 시작) 가 전사 저장의 외래키 선행 조건. CORS 는 `CORS_ALLOWED_ORIGINS`. 응답 전 필드 문자열화(`StrField`, §7.3 정본과 어긋남 — 미결). 게이트웨이 알림 3종(검색 중·콜 가드·필요서류)은 코드만 있고 꺼 둠 — **프론트 수신 뒤에 켠다**(09-15 결정). ⚠ **운영·Neon 스키마 적용 여부 미확인** — 이 머신에 Neon 유효 자격증명·AWS 키가 없다. ngrok 터널 인계는 닫았다 |
