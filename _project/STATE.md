...
> ⚠ **모델 구성(dense·리랭커·NER)은 여전히 로컬 측정뿐이다** — 모델은 전용 GPU EC2 에 올리기로 했고(`decisions/121`),
