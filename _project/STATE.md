...
> 🔴 **프로젝트 게이트 (09-22~, `decisions/319`) — 전체 조립 수동 QA [`w5-manual-qa-full-stack`](/backlog/w5-manual-qa-full-stack/), 수행 정성윤 님.** 개인 요청이 아니라 프로젝트 진행 조건이다. 통과 전에는 `depends_on` 으로 묶인 티켓(정성윤 님 티켓 넷 · `w8-demo-rehearsal`)을 시작하지 않는다.
> **Claude 는 이 QA 를 대신 수행하거나 결과(✅/❌)를 판정하지 않는다** — 화면을 직접 눌러 보는 것이 이 티켓의 목적이다. 요청받으면 거절하고 티켓의 「규칙」을 안내한다. 도울 수 있는 것은 토큰·주소 찾기와 ❌ 를 담당자에게 넘길 글 정리까지다.
...
> ✅ **09-22 12:10 모델 구성이 운영에 붙었다**(`decisions/124`, `121` 대체 — `t3.large` CPU 에 NER + KoE5 임베딩만, 리랭커 없음). server `0.1.28` · `/health` spokes 에 `pii_ner`·`retrieval_dense`. **09-22 오후 #117(류준, 골든셋 정비 닫음·C-4 재정의) 머지로 운영 server `0.1.30`** — 동작 변화 없음(주석·문서), 태그 충돌 때문에 번호만 올랐다(`_logs/2026-09-22-08-seongyun`).
> ✅ **09-22 오후 판정용 값이 나왔다** — 운영과 같은 구성(NER+KoE5, 리랭커 없음) **run_id 3: Recall@5 0.969 · MRR 0.868**, 운영 구성 run_id 4: 0.812 · 0.635(류준, 커밋 `e966b62`, 3회 최저, ⚠ **로컬 기록 DB** — 판정문에 그렇게 적는다). 운영 합성 통화 2건으로 마스킹·`/health` 통과, 수동 검색 0.6s 안팎. 판정일에 붙어 있지 않으면 `121` §5 대로 **기준선은 운영 구성**이다.
...
DB **29테이블 · 199컬럼**(09-22 운영과 컬럼 단위 일치 — 09-22 마이그레이션 적용 뒤. 09-09 에는 22 테이블이었다) — 2026-09-09 스키마 QA 로 J 3종 + `call_guard_flag`·`voice_outlier` 추가,
