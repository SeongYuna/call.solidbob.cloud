...
| `ai/` | 류준 | **09-22 현재.** 검색: **운영 구성(규칙+BM25) 0.812 / MRR 0.635 · 모델 구성(KoE5 dense + bge 리랭커 + 규칙+NER) 0.969 / 0.914** — 골든셋 `v1-150.1`, 로컬 기록 DB run_id 1·2(아래 「실측값」). **모델 구성은 운영에 없다**(`decisions/121`, GPU EC2 미생성) — 「로컬 측정」 꼬리표 유지. C-5 누락 0(n 28, P1~P7 전부 표본) · F-2 1.0(n 99, 규칙표 상한) · 컴플라이언스·콜 가드 1.0(자기충족 상한) · B-4 kanana 로컬 1회 출하 환각 0/480(원출력 환각은 포트가 원출력을 주지 않아 측정 불가). **하네스**: 생성(B-4)·A-5 가 리포트에 「측정 불가 + 사유」로 나온다(`w6-harness-silent-metrics`). **골든셋**: AI Hub 원문 14건을 바꿔 썼다(`decisions/212`, 라벨 불변 — 이전 수치와 항목 단위 비교 불가). **`ai/apps/model_serving`** — 모델 HTTP 표면(`decisions/213`, `MODEL_SERVICE_TOKEN` fail-closed). 서버 어댑터는 장민석 님 몫(`w6-server-remote-model-adapter`)이고, CI `ai` job 에 fastapi·httpx·uvicorn 이 없어 **표면 테스트는 CI 에서 skip** 된다. GS-205 원인: nori 품사 필터 없음(`w6-golden-set-loose-ends`, 채택은 결정 필요). 테스트 ai 496 · server 1,257 · scripts 15 · 계약 3+4 KEPT(09-22). 남은 막힘: server 이미지 태그(아래) · GPU EC2·Chirp 3 IAM(정성윤) · 품사 필터·B-6 문턱·C-4·용어(팀) · 음성·대본 사람 검토 · D-5 서버 저장은 새 API 필요(`w7-d5-server-wiring`, 장민석) |
...
| **server 이미지 태그 미상향** | 류준·정성윤 | 09-22 `ai` 브랜치가 서버 이미지에 들어가는 코드(`ai/` 라이브러리 · `eval_run_repository.py`)를 바꿨는데 `newTag` 가 `0.1.24` 그대로라 **PR 을 올리면 `tag-check` 가 빨간불**이다. `0.1.25` 는 이미 레지스트리에 있다(다른 브랜치가 구움) — **비어 있는 다음 번호를 골라 같은 PR 에서 올린다** |
...
`ai/` 모듈(09-22 아홉 — `model_serving` 추가, `decisions/213`): `retrieval` · `evaluation` · `call_guard`(C-6) · `voice_signal`(D-5) · `compliance`(C-1~C-4) · `generation`(B-4~B-6) · `pii_ner`(C-5 P6·P7) · `postcall_summary`(D-1·D-2) · `model_serving`(원격 추론 표면).
