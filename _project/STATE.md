...
| 영역 | 주 담당 | 상태 |
|---|---|---|
| `server/` | 장민석 | 엔드포인트 15개 · 테스트 290 + integration 3(PR #61 재검증치) · 스포크 4종(`masking`·`closure_gate`·`retrieval`·`trigger`) — **501 은 이제 없다**(트리거 배선 09-10). `POST /hub/calls`(통화 시작, `decisions/301`) 가 전사 저장의 외래키 선행 조건. 커넥션은 `DATABASE_URL` 우선(운영 주입 방식) · ES 인덱스 없으면 503. CORS 는 `CORS_ALLOWED_ORIGINS`. 응답 전 필드 문자열화(`StrField`, §7.3 계약 정본과 어긋남 — 미결). **09-14 게이트웨이 마무리**: 추천 요청이 `received_at_ms`(게이트웨이 도착 시각)를 받아 트리거 발동 시각이 실시간 경로에서 측정값 · ES 연결 실패도 503 · 게이트웨이 `speaker=auto` 모노 화자 분리(`decisions/303`, **실제 구글 미검증**) · 「검색 중」 신호는 코드만 있고 꺼 둠(프론트 수신 대기). 테스트 server 327 · ai 194 · gateway 92, 이미지 태그 server 0.1.4 · gateway 0.1.2. **진행 중**: 로컬 서버(Neon + 로컬 ES)를 ngrok 으로 조서희에게 연다 — authtoken 대기(`w3-api-tunnel-handoff`) |
