...
**5주차** / 8스프린트 (2026-08-20 ~ 2026-10-27). 1주차 목표 6개 전부 달성.
...
> ⚠ **모델 구성(dense·리랭커·NER)은 여전히 로컬 측정뿐이다** — 모델은 운영 노드에 CPU 로 싣기로 했고(`decisions/124`, `121` 대체 — 09-30 까지 붙으면 모델 구성, 아니면 운영 구성으로 판정),
> 판정일까지 그것이 떠 있지 않으면 **기준선은 운영 구성**이다.
...
`ai/` 모듈(09-21 여덟): `retrieval` · `evaluation` · `call_guard`(C-6) · `voice_signal`(D-5) · `compliance`(C-1~C-4) · `generation`(B-4~B-6) · `pii_ner`(C-5 P6·P7) · `postcall_summary`(D-1·D-2).
