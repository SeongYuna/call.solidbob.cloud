...
| **운영 `document` 테이블 비어 있음** | 정성윤·류준 | `scripts/seed_documents.py` 를 운영 RDS 에 안 돌렸다(런북 17-3). **안 돌리면 욕설이 걸린 고객 발화의 전사 저장이 FK 위반 500** — C-6 배선 이미지 배포 전에 돌린다 |
