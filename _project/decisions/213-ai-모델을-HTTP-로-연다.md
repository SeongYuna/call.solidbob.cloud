...
- 날짜: 2026-09-22
- 작성: 류준 (`2xx` 번호대)
- 상태: **철회 — [124](124-09-30-판정은-모델-구성으로-시도한다-모델은-운영-노드에-CPU-로-싣는다.md) 로 대체됐다(같은 날).** 124 가 모델을 운영 노드에 CPU 로 싣기로 해 원격 표면이 필요 없어졌고, 024·105(ai 는 라이브러리)가 그대로 산다.
  **구현은 되돌렸다** — 코드는 커밋 `992b475`(머지 `121157e`)에 있고 되돌림 커밋으로 빠졌다. 되살리려면 그 되돌림 커밋을 `git revert` 한다.
  아래는 철회 전 원문이다(절대 원칙 8). 원래 상태 줄: 채택 — **`ai/` 쪽 표면만 구현됐다.** 서버 원격 어댑터(`w6-server-remote-model-adapter`, 장민석)와 GPU 인스턴스(`121`)는 없다.
