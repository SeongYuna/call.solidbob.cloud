...
> ⚠ **09-18 갱신 — §15 를 먼저 읽는다.** §1~§14 는 09-17 아침 기준이고, 그 뒤 main 에 37커밋이 들어왔다(개명 `gateway`→`call-mediator` · C-5 규칙 보강 · 컴플라이언스 배선 · 합성 통화 24건 E2E · `decisions/208`·`210` · 운영 EC2 는 `t3.large` 이고 **자동 중지가 안 걸려 있다**). §15 가 앞 절 6곳을 정정한다.
...
## 15. 09-18 갱신 — 09-17 ~ 09-18 에 main 으로 들어온 37커밋 반영

> §1~§14 는 **09-17 아침 `PM c0b828b` 기준**이다. 그 뒤 PR #97~#102 로 37커밋(296파일)이 main 에 들어왔고 09-18 아침 `PM` 을 `a387469` 로 맞췄다.
> 근거: `_logs/2026-09-17-01~04-seongyun` · `2026-09-17-01·02-ryujun` · `2026-09-18-01·02-ryujun` · `decisions/115·208·209·210` · 티켓 `w5-*` 5건 · 미결 항목 새 절 5개.
> **아래가 앞 절과 다르면 아래가 맞다.** 앞 절의 해당 문장은 고치지 않고 여기서 정정한다.

### 15-1. 앞 절을 뒤집는 정정 6건

| 앞 절 | 정정 |
|---|---|
| §1·§3·§7-3 ① 「운영 C-5 는 규칙만이라 골든셋 4건 뚫림」 | **규칙만으로 골든셋 v1-150 누락 4→0·오분류 1→0**(류준 09-17, server `0.1.15`; 09-18 `0.1.18` 추가 보강). 오류 곡선 규칙만 0·2·3·4·4(전 4·5·5·5·5). AI Hub 실제 발화 과잉 459→44. **`0.1.15` 이상이 운영에 뜨면** 규칙만으로도 골든셋 기준 누락 0 이다. 남은 한계: 문맥 없는 이름(「김민준 그 사람이」)은 NER 몫(`208`) |
| §7-3 ② 「자동 중지 cron 상태 확인 필요 — 문서마다 다름」 | **안 걸려 있다.** 류준 09-17 운영 점검: AL2023 에 `crontab` 없음, **3일 연속 가동**. 런북 21-1 명령이 안 먹는다(추정). EIP 도 없어 재기동 시 IP 가 바뀐다 — **EIP 부터 붙이고 자동 중지를 systemd timer 로** |
| §7-1 「운영 EC2 등급 실물 기록 없음」 | **`t3.large`(2 vCPU · 7.8GB, 09-17 사용 3.6GB), 결정 기록 없음.** GPU 아님. AMI 0개 · EBS 30GB 비암호화 · Name 태그 없음 · 안 붙은 SG `launch-wizard-2`(22 전체 개방) |
| §11-6 #1 「컴플라이언스 검사를 아무도 안 부른다」 | **콜 미디에이터 `0.2.1` 이 상담원 확정 발화마다 `POST /hub/compliance-checks` 를 부르고**, 서버 `0.1.18` 이 `compliance_flag` 를 저장한다(전엔 응답만). 남은 것: WS 방송 `announceCompliance` 기본 **false**(대시보드 파서가 `compliance` 를 안 받는다) · 화면 경고는 아직 프론트 로컬 규칙 |
| §12-2 「훈련 방침이 두 갈래로 열려 있다」 | **`decisions/210` 채택 — 모델을 학습하지 않는다. 데이터는 S3 측정용.** `w5-classifier-ner-benchmark` 는 「측정 불가 — 학습 안 함」으로 **done**. `w3-a5-translation-spike`·`w3-call-temperature` 메모는 실제 막힘으로 정정됨 |
| §11 전체 · §7-1 「게이트웨이 `services/gateway`」 | **`decisions/115` 개명 — `services/call-mediator/`, 이미지 `callguard-call-mediator`, 경로 `/call-mediator/*`, 키 `CALL_MEDIATOR_*`, 변수 `VITE_CALL_MEDIATOR_*`, 오버라이드 `?call_mediator=`.** 저장소·CI·프론트는 끝났고 **운영은 아직 `/gateway/*`** |

### 15-2. 새로 된 것 (09-17~18)

- **C-5 규칙 보강**(류준, 장민석 검토 전): 이름 네 갈래(문맥·자기소개·호칭·이름만 답함) + 가족 호칭 · 도로명 뒤 건물번호 · 집 전화 P4 · 어미·조사 과잉 제거 · 서버 골든 테스트 v1-50→v1-150(**전에는 누락 4건이 CI 초록이었다**). `test_pii_gaps_0918.py`.
- **컴플라이언스**: 콜 미디에이터 호출 + `compliance_flag` 저장 배선 + `compliance_rule` UPSERT(severity 전부 `medium` — 등급 정의 없음). C-6 위기 신호·C-1/C-3 규칙 보강(실제 발화 과탐지 증가 0).
- **필요서류**: `closure_rule` 25→29(305 EXCLUDED 중 2.6·4.5·4.9·4.12 되살림, 갈래 공통 서류만 필수) · 콜 미디에이터가 1순위 카드가 아니라 **카드 순서대로 규칙 있는 첫 조항**까지(상한 5). 추천 후보에서 POLICY 조항 제외(BM25 MRR 0.659→0.666).
- **합성 통화**(`decisions/209` 제안): 페르소나 9명·대본 24건(SYN-001~024, 라벨 P1~P7 전부 채움) · Google TTS Wavenet 어댑터(키만 넣으면 됨, 월 900,000자 앱 가드) · **E2E 왕복 검사 `scripts/persona_sim/e2e_check.py`** — 24건 ✅3/❌21 → 결함 6종 수정 후 **✅22/❌2**. 남은 ❌2 = 검색 순위(BM25 상위 5에 정답 없음 → 임베딩 몫).
- **상담원 로그인**(조서희·PR #99, server `0.1.16`): 로그인·로그아웃 화면 + 확인·이름 목록 엔드포인트, 토큰 발급을 이름으로.
- **운영 대시보드 라이브 연결**(정성윤 09-17): `call.solidbob.cloud` 번들에 `VITE_CORE_API_URL`·WS 주소·데모 주소를 넣어 강제 배포 → 운영 `/gateway/ws` 63ms 열림, `mode=live connected=true`. **⚠ 개명 뒤 변수 이름이 바뀌어 다시 넣어야 한다**(115 전환 3단계).
- **골든셋 오답 2건 원인 분석**(GS-204 쌍둥이 조항 채점 기준 · GS-205 맥락 없음+nori 토큰화) — `206` 「원인 미분석」 닫힘.
- 09-17 운영 점검: 파드 4개 Running · 서버 24h 5xx 0 · 콜 미디에이터 48h 실패 0 · 문 4곳 잠김 · **바깥 실제 접속 0건**.
- 테스트 규모: server **1,007** · ai **416** · call-mediator **127**. 미결 열린 항목 **120**(닫힌 것 포함 전체 증가) · 티켓 **180**(done 151).

### 15-3. 새로 생긴 남은 일 (앞 절에 없던 것)

**🔴 인프라 — 실측으로 급해진 것**
1. **EIP 부착 → 자동 중지(systemd timer)** — 3일 연속 가동 중. 예산 방어선이 없다(§7-5 의 $142 전제가 깨져 있다).
2. **AMI 스냅샷 · EBS 암호화 · SG `launch-wizard-2`(22 전체 개방) 삭제 · Name 태그 · t3.large 결정 기록**.
3. **개명 운영 전환 5단계**(`w5-call-mediator-rename`): EC2 시크릿 복사 → Vercel 새 변수 → 룰셋 `gateway`→`call-mediator` → 배포 뒤 `/call-mediator/health` → 옛 오브젝트·변수 삭제. **룰셋을 안 바꾸면 다음 PR 이 영원히 pending**.
4. **`decisions/208` 합의** — torch CPU 휠을 이미지에 넣을지 · 모델 파일 위치(hostPath/PVC vs S3) · **t3.large 에서 재측정**(류준 값은 M5 맥: NER 663MB·p95 32ms, 임베딩 1,957MB·p95 68ms, 둘 합쳐 2,286MB). 권고 B: NER+임베딩만, 리랭커는 GPU 뒤.
5. **Google TTS 키 발급**(`scripts/persona_sim/TTS.md §1` — API 제한 TTS 하나, 콘솔 할당량 낮추기).
6. 0.1.18·0.2.1 릴리스 성공 여부 확인(이 머신에 `gh` 없음).

**🟠 서버·콜 미디에이터**
7. `POST …/close` 가 `ended_at`·`status` 를 안 바꾼다 — 통화 종료 상태를 쓰는 경로 없음(24건 전부 경고).
8. 장민석 검토 요청 2건 — C-5 규칙 네 갈래·`_NOT_NAMES` 근거 · 09-18 `0.1.18`(EXCLUDED 되살림 · 가족 호칭 · `compliance_rule` UPSERT 위치·severity).
9. `announceCompliance` 켜기 — 대시보드 파서가 `compliance` 를 받은 뒤.
10. GS-204 채점 기준 결정 기록(3.2 정답 추가 / 쌍둥이 통합 / README 규약) · GS-205 nori `_analyze` 확인 후 앞 발화 보강.
11. `compare_retrievers.py` 가 항목별 상위 k 를 안 남긴다 — 다음 측정 때 기록.

**🟠 프론트(`apps/`)**
12. **랜딩·상담원 박스가 `call_id` 를 각자 만든다** → 고객 발화가 상담원 화면에 안 닿는다. ID 공유 + ID 로 구독·필터(`applyTranscript` 가 `call_id` 대조 없이 덮어씀).
13. 못 읽는 메시지 하나가 `connected:false` 를 만든다(소켓은 열려 있음).
14. `AgentCallBox` 모달이 합성 통화 라이브 보기를 가린다.
15. 컴플라이언스 경고를 프론트 로컬 규칙(`detectComplianceRisk.ts`)에서 서버 결과로.
16. 개명으로 `?call_mediator=` · 저장 키 변경 — 브라우저에 남은 옛 오버라이드 무시됨(팀 공지).

**🟡 문서·결정**
17. `decisions/208`·`209` 는 «제안» — 합의해 상태를 바꾼다.
18. 09-14 히어로 「상담원에게 전화 걸기」 변경이 진행 기록에 없다(커밋·주석에만).
19. 중복 티켓 `w1-platform-landing` ↔ `w3-platform-landing-refresh`.
20. 운영 합성 통화 1건(SYN-010) 동의 — 운영 DB 에 `syn-` 행이 남는다.
21. `ai/apps/training/` 처리(보관·이동) — `210` 이 지우지 않기로.
22. 4.20·4.8·3.3·6.12 는 규칙표에 «의도적으로» 없음 — 대본 SYN-006·007·012 는 판정 0건이 정상.

### 15-4. 앞 절에서 닫힌 것

- §6-1 `w5-classifier-ner-benchmark` → done(측정 불가). §12-2 훈련 방침 → `210` 채택.
- §11-6 #1 컴플라이언스 호출부 → 콜 미디에이터가 부른다(방송·화면만 남음). §13-2 «합성 대본을 하네스에 올릴지» → 올리지 않는다(E2E 검사가 그 역할).
- §7-3 ② «cron 확인 필요» → 없음으로 확정. §7-1 «EC2 등급 미기록» → t3.large.
- §1 «C-5 운영 4건 뚫림» → 규칙 보강으로 골든셋 기준 0(배포 뒤 성립).
- §14-1 #11 중복 티켓은 그대로이고, 여기에 `w1-platform-landing` ↔ `w3-platform-landing-refresh` 가 하나 더 있다.

### 15-5. §7-4 · §9 갱신

- §7-4 첫 줄 「GPU 를 켤지」는 **`208` 이 답을 냈다** — CPU `t3.large` 에서 NER+임베딩(약 2.3GB)만 켜고 리랭커는 GPU 뒤. 남은 판단은 «이미지에 torch 를 넣을지 · 모델 파일을 어디 둘지 · t3.large 재측정».
- §9 멘토 질문 1번은 «GPU 냐 로컬 수치냐»가 아니라 **«CPU 노드에서 NER+임베딩만 켜는 절충이 발표에 충분한가»** 로 바꿔 묻는다. 4번(합성 통화)은 `209` 대로 «데모·점검 전용, 수치는 상한 표기»로 묻는다.
