...
/**
 * 콜 미디에이터 접속 허용 규칙 — `/ws`(자막을 받는다)·`/ingest`(STT 과금을 쓴다) 둘 다에 건다.
 *
 ...
 * - `/ingest` = `CALL_MEDIATOR_INGEST_TOKEN`. STT 과금을 쓰는 문이라 **진짜 비밀**이다. 생산자(서버 쪽 스크립트)만
 *   갖고, `Authorization` 헤더로만 받는다 — URL 에 실리면 접근 로그·기록에 남는다.
 * - `/ws` = `CALL_MEDIATOR_VIEW_TOKEN`. 대시보드는 공개 사이트라 브라우저가 내는 토큰은 번들에서 누구나 읽는다.
