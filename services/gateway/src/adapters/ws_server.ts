 ...
 * | `WS /ws[?call_id=]` | 대시보드 | ← `{type, payload}` JSON (마스킹된 결과만) |
 ...
 *    `/ingest` 는 `GATEWAY_INGEST_TOKEN` 을 `Authorization: Bearer` 로만(URL 에 비밀을 싣지 않는다).
...
import { bearerToken, decideAccess, isLoopback } from "../domain/access.ts";
...
export function createGatewayServer(deps: GatewayServerDeps): Server {
  const ingestWss = new WebSocketServer({ noServer: true, maxPayload: 1 << 20 });
  ...
  const heartbeat = setInterval(() => {
    for (const wss of [ingestWss, dashboardWss]) {
  ...
  server.on("upgrade", (req: IncomingMessage, socket: Duplex, head: Buffer) => {
    ...
    const door = path === "/ingest" ? "ingest" : path === "/ws" ? "view" : null;
    if (door === null) {
      socket.end("HTTP/1.1 404 Not Found\r\n\r\n");
      return;
    }
    const trusted = (deps.isTrustedAddress ?? isLoopback)(req.socket.remoteAddress);
    const header = bearerToken(req.headers.authorization);
    // 과금 문의 비밀은 헤더로만 받는다. 대시보드 토큰은 비밀이 아니라 URL 도 받는다.
