...
| `compliance` (C-1~C-4) | 대체 표현 출처(`<DOM>-MANUAL-1.4` 및 각 위반 조항 — C-1~C-4 공통) | C-1~C-4 탐지 규칙 |
...
| C-4 | 근거 없는 안전 보장 — 의학적 안심 발언 등 (2026-09-22 재정의, `decisions/211`) | `DASAN-MANUAL-4.1` |

> 권장 대체 표현은 C-1~C-4 **공통 출력**이다 — 모든 탐지에 `alternative_source`(MANUAL-1.4 및 각 위반 조항)가 붙는다.
