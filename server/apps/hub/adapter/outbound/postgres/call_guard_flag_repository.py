...
class PostgresCallGuardFlagRepository(CallGuardFlagRecordPort):
    ...
    async def record(self, call_id: str, segment_id: int, flags: tuple[CallGuardFlag, ...]) -> None:
        if any(f.span is None for f in flags):
            ...
            raise CallGuardFlagSpanMissingError("위치(span) 없는 콜 가드 신호는 저장하지 않는다 — 스포크가 span 을 채워야 한다")
        ...
        async with self._connect() as conn:
            async with conn.cursor() as cur:
                try:
                    ...
                    # `source_doc_id` 는 서브쿼리라 FK 로 실패하지 않는다 — 남은 외래키는 transcript_segment 하나다.
                    # 2026-09-20 운영에서 이 위반이 그대로 터져 **500** 이었다. 호출 순서 문제라 4xx 로 드러낸다
