...
def get_agent_directory_port(request: Request) -> AgentDirectoryPort:
    settings = request.app.state.settings
    if not settings.postgres_configured:
        raise HTTPException(
            status_code=status.HTTP_501_NOT_IMPLEMENTED,
            detail="PostgreSQL 이 설정되지 않았습니다 — 상담원 목록을 볼 수 없습니다",
        )
    return PostgresAgentDirectoryRepository(build_connection_factory(settings))
