...
# TERM 장 번호 → **운영 유형표**(`decisions/323`)의 값. 표 밖의 장은 투표하지 않는다.
#
# ⚠ **이름은 운영이 DB 에 저장하는 값과 글자까지 같아야 한다**(2026-09-23). 전에는 이 표가 AI Hub
# 카테고리 이름(「대중교통 안내」·「생활하수도 관련 문의」…)을 썼고 운영은
# `server/apps/postcall/domain/services/inquiry_rules.py` 의 짧은 이름을 써서 **같은 장을 가리키는데
# 글자가 달랐다** — 골든셋 채점이 정확 일치라 0.0 이 나왔고(09-22 골든셋을 운영 쪽에 맞춰 고쳤다),
# 모델 스포크를 켜는 순간 운영 DB 에도 두 벌이 섞여 들어간다. 그래서 여기를 운영 쪽으로 맞춘다.
# **이번에는 이름만 맞춘다** — 어느 장이 투표하는지는 그대로다. 6장(재난·생계 지원금)은 전처럼 표 밖이라
# 투표하지 않는다(운영 규칙표에는 그 유형이 있다 — 넣으려면 표본으로 재고 따로 결정한다).
