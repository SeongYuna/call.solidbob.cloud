...
- [x] 태그 올림 · 배포 뒤 `/health` 가 여전히 `locked`
...
- server 태그 ~~`0.1.33`~~ → **`0.1.34`**(#129 가 `0.1.33` 을 먼저 써서 머지 때 정성윤 님이 올렸다).

## 2026-09-22 — 완료 (배포 확인)

PR #131 머지(07:02) → release 성공(server `0.1.34`). 운영 `/health` 의 `ingest_guard` **`locked`** · 토큰 없는 `POST /hub/calls`·`/hub/routing-decisions` **401**.
