---
title: "services/call-mediator 신설 — 전화 사업자·STT 키 없는 개발용 테스트 콜"
assignee: "조서희"
role: "app"
status: "done"
sprint: 3
...
`services/call-mediator`(Node.js/Express/ws)를 새로 만들었다 — `hubClient.js`가
...
`apps/dashboard/src/lib/ws/types.ts`의 `callMediatorUrl()`에 `?call_mediator=<wss URL>` 쿼리 기반
런타임 오버라이드를 추가했다 — Vercel 빌드타임 환경변수 없이도 배포된 대시보드를
라이브 모드로 전환할 수 있다.

## 왜

담당 경계상 `services/call-mediator`는 정성윤 몫이다(`_project/decisions/012`). 포트폴리오
...
- [x] `/dev` 페이지 → 콜 미디에이터 → 배포된 실제 백엔드(`server.solidbob.cloud`) →
...
- **정식 A-1/A-2(실제 전화망 연동)는 여전히 미착수다.** `/dev`는 브라우저 STT
  품질이라 "콜 미디에이터→hub→대시보드" 배선 확인용이지, STT 품질 측정용이 아니다.
