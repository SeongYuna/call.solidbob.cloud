---
title: "트리거 발동 시각을 실제 도착 시각으로 — 게이트웨이 received_at_ms"
assignee: "장민석"
role: "ai"
status: "done"
sprint: 4
...
paths:
  ...
  - "services/gateway/src/app/ports.ts"
...
([미결](/open-items/) 2026-08-27 항목). 실시간 경로가 생겼으니 게이트웨이가 STT final 을 **받은 시각**
(`received_at_ms`, 통화 시작 기준 ms)을 `POST /hub/recommendations` 에 실어 보내고, 트리거가 그 값을 쓴다.

## 완료 조건

- [x] 게이트웨이가 final 을 받은 시각을 `received_at_ms` 로 보낸다 (테스트)
