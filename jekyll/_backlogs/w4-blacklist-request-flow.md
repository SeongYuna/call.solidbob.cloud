...
- [x] **서버 어댑터·라우터** — 장민석 님 `w4-blacklist-api`(server 브랜치) 에서 끝났다(2026-09-14, `server` 브랜치 `b1dade6` · main 머지 전)
- [x] §7.3 계약에 J 등록 — 같은 커밋이 `plan.md` 7.3절 「허브 HTTP 표면」 에 블랙리스트 API 를 올렸다
...
## 2026-09-14 — 남은 서버 몫은 다른 티켓에서 끝났다

J-1·J-2·J-4 의 저장·HTTP 는 장민석 님이 `w4-blacklist-api`(server 브랜치) 로 만들었다(요청·목록·승인/반려·등록 목록·해제,
고객은 발신 번호 HMAC, 승인은 관리자 로그인). **이 티켓에서 따로 만들지 않는다** — 겹치면 나중에 시작한 쪽이 물러난다(§4).
보드에서 완료가 두 번 세어지지 않도록 **실제 서버 작업은 그 티켓의 것**으로 본다. ⚠ 그 브랜치는 아직 main 에 들어가지 않았다.
