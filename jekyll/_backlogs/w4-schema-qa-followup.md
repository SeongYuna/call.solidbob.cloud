...
> **2026-09-11 정성윤 — ①·② 를 이 티켓에서 뗐다.** ① 은 운영 배포(이미지 `0.1.2`)를 막고 있어
> [w4-segment-composite-upsert](/backlog/w4-segment-composite-upsert/) 로 옮겨 정성윤이 고쳤다
> (`decisions/302` — `server/` 는 누구나 고친다). ② 는 장민석의 `POST /hub/calls`
> ([w3-call-start-api](/backlog/w3-call-start-api/), `decisions/301`)로 이미 풀렸다.
> **이 티켓에 남은 것은 ③ 하나다** — 담당·상태는 그대로 둔다.
...
- [x] ③ 이후 골든셋에 F-2 케이스를 넣을 수 있다 → 류준에게 알린다 — [미결](/open-items/) 「콜 미디에이터 마무리에서 남은 것」 에 적었다
