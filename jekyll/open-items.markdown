...
### J-5 배정 판정(`POST /hub/routing-decisions`)을 누가 부르나 (신규, 2026-09-15, 정성윤)

조서희 님이 프론트 점검에서 짚은 6개 중 **아직 안 닫힌 하나**다(보존 정리는 `decisions/113` 으로 닫혔다).

2026-09-15 코드 확인 —

- 엔드포인트는 **있다**(`routing_decision_router.py`, `decisions/313`)
- **인증이 없다** — 이 라우터에는 `require_admin` 이 없다. 통화 시작(`POST /hub/calls`)과 같은 상태다
- **게이트웨이는 부르지 않는다** — `services/gateway` 가 서버로 보내는 것은 `POST /hub/calls` 하나뿐이다
  (`adapters/hub_http.ts:33`). 배정 판정 호출은 코드에 없다
- **프론트는 관여하지 않는다** — 화면이 부르는 API 가 아니다(교환기·게이트웨이가 부른다)

- [ ] **데모에서 언제 부를지** — `/dev` 테스트 콜이 통화 시작 직후 이어서 부르게 할지, 아니면 배정 판정
      자체를 데모 범위에서 빼고 「엔드포인트는 있다」로 남길지. **정성윤(게이트웨이 코드)**
- [ ] 부르기로 하면 **인증도 같이 정한다** — 지금은 토큰 없이 열려 있다. 통화 시작 API 와 묶어서 보는 게 맞다
