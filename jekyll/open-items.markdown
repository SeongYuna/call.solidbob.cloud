...
### 운영 DB 에 저장소에 없는 테이블이 하나 있다 (신규, 2026-09-15, 정성윤)

운영 `assist` DB 에 **`blacklist_entry_expiry_change`** 가 있는데, 이 이름이 **저장소 어디에도 없다** —
`db/schema.sql`(26개) · `db/migrations/` 두 파일 · `server/`·`ai/` 코드 전수 검색 **0건**(2026-09-15 확인).

지금까지 드리프트는 늘 「저장소에 있는데 운영에 없다」 방향이었다(09-14 마이그레이션이 그래서 생겼다).
**이건 반대 방향이라 성격이 다르다** — 운영에만 있는 것은 어떤 절차로도 재현되지 않고,
인스턴스가 사라지면 같이 사라진다.

- [ ] **누가 왜 만들었는지** — 손으로 만든 실험 흔적인지, 어떤 브랜치의 스키마 변경이 운영에만 먼저 들어간 것인지.
      이름으로 보면 블랙리스트 만료 연장 이력 같다 → J-4 흐름을 만든 사람(조서희 님·장민석)에게 먼저 묻는다
- [ ] 행이 0 이고 아무도 모르면 지운다. **행이 있으면 지우지 않는다** — 무엇을 지우는지 모르는 채로 지우지 않는다(절대 원칙 8)
- [ ] 남기기로 하면 `db/schema.sql` 과 마이그레이션 파일에 **역으로 반영**한다. 그래야 다음 재구축에서 살아남는다

확인: `SELECT count(*) FROM "blacklist_entry_expiry_change";`
