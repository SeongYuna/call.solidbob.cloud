...
- [ ] **운영 첫 검색 요청이 3,300ms — 예열인지 확인 — 정성윤** — 모델 탑재 뒤 첫 합성 통화 SYN-010 의 추천 2건 `internal_latency_ms` 3,300 · 2,118(상한). 바로 뒤 SYN-008 8건은 394~778. 첫 요청 예열로 보이지만 가설이다 — 서버 로그의 `retrieval_ms` 로 가른다. 예열이 맞으면 기동 때 더미 질의 한 번이 해법 후보(판정 규칙 124 ⑥ 「p95 1,000 넘으면 임베딩 끔」을 첫 요청 하나로 적용하지 않도록)
