...
- [ ] **정성윤 님께 확인 — 백엔드 티켓 둘을 장민석이 넘겨받아도 되는지** (2026-09-22 추가) — `w7-j5-routing-caller`(콜 미디에이터가 통화 시작 직후 배정 판정 호출 — 서버 문은 09-22 에 잠갔다) · `w6-deploy-schema-precheck`(`release.yml` 에서 운영 스키마 대조 후 어긋나면 배포 중지). 둘 다 `todo` 라 넘겨받을 수 있지만 오늘 운영 작업이 활발해 겹칠 수 있어 먼저 묻는다
