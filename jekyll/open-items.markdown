...
- [ ] **운영 RDS 에 `document` 행이 없다 — C-6 배선 이미지를 올리기 전에 적재한다 (신규, 2026-09-14)** — 전사 수신이 이제
  고객 확정 발화에 콜 가드를 돌려 `call_guard_flag` 에 쓰는데, 근거 조항(`DASAN-MANUAL-5.x`)이 외래키다. **비어 있으면 욕설·위기
  신호가 걸린 발화의 `POST /hub/transcripts` 가 500** 이다(자막 행은 먼저 저장돼 남는다). 절차는 런북 17-3 — `scripts/seed_documents.py`.
  **정할 것**: 누가 언제 돌리는가(배포 담당 정성윤 님 vs 류준). 서버 이미지 태그는 `0.1.4` 그대로 두었다 — 레지스트리에서 `0.1.4` 를
  확인하지 못해(없거나 조회 실패) 올리지 않았다. **이미 발행된 태그라면 머지 뒤 `release.yml` 이 실패하므로 올려야 한다**
