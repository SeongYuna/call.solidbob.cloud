...
- [ ] **C-5 중간 자막(interim)에 번호 앞자리가 가려지지 않고 나간다 — 류준·장민석** — 표본 6건 중 5건: 카드 `9410 0000`·전화 `010 0000`. 확정 자막·DB 는 가려진다(화면에만). 번호가 다 들어오기 전이라 패턴이 안 맞는 것으로 보인다(가설). 절대 원칙 3 「애매하면 가린다」
- [ ] **읽기 경로 인증 없음 — 운영에서 열려 있다 — 장민석** — `GET /hub/calls`(토큰 없이 12건 · 실제 음성 테스트 통화 포함 · 고객 HMAC · `?customer_id=` 필터) · `/transcript` · `/record` · `GET/POST/PATCH /hub/knowledge-gaps`(설명이 **마스킹 없이** 저장) · `POST /hub/search`. 기존 미결 「남은 것은 읽기 경로」와 같은 건 — 09-30 전에 닫히지 않은 가장 큰 구멍
