...
- [ ] → 2026-09-22 **값은 정했다 — 류준 결정, 팀 회의 합의 없음**(`decisions/215`): 사전 등록 후 보류 표본 24건으로 한 번 재서 dense 1순위 < 0.67 채택(걸러냄 12/24 · 정답 손실 1, 둘 다 경계값). 코드 반영(`FallbackRetriever` — 기권 시 BM25 로 안 내려감), **운영 효과는 임베딩 탑재(`124`) 뒤**. **남은 것**: 팀이 뒤집을지 · 「관련 문서 없음」 vs 「소관 안내」 카드 형식과 화면의 요청 단위 빈 상태 표시(지금은 누적 카드 0장일 때만 「관련 문서가 아직 없습니다.」 — 조서희 님) · 문턱이 못 가르는 타 기관 하드 네거티브(점수 밖 신호) · **B-6 문턱 — 팀 결정** — `w5-b6-no-answer-threshold` 측정 결과: 세 계열 모두 문턱 하나로 못 가른다. dense 0.67 안팎이 후보(손실 1/94 로 24건 중 15 걸러짐), BM25 는 권하지 않음. 타 기관 문의에 「관련 문서 없음」 카드가 맞는지 「소관 안내(MANUAL-2.2)」 카드가 맞는지도 프론트 카드 형식과 함께(조서희 님). 채택되면 **새 B-6 케이스로 재측정한 뒤** 어댑터에 규칙(새 티켓).
