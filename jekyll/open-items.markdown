...
- [ ] **운영 첫 검색 요청이 3,300ms — 예열인지 확인 — 정성윤** — ◐ **2026-09-22 13:01 재생기로 SYN-010 을 다시 흘렸다**(`syn-prod-syn-010-seongyun-130124`, 같은 파드·모델 예열 뒤): 추천 2건 `internal_latency_ms` **1,352 · 655**. 첫 추천이 여전히 1,000 을 넘는다 — 수동 검색(0.6s)보다 두 배 이상이고, 미디에이터 경로에서는 같은 2 vCPU 위에서 **NER 마스킹(확정 자막마다)과 임베딩 검색이 동시에** 돈다. 가설: 첫 추천이 앞 자막들의 NER 추론과 CPU 를 다툰다. 그래서 「예열」보다 **CPU 경합** 쪽이다 — `retrieval_ms`·`generation_ms` 를 `/ws`·DB 로 내보내야 가른다. 판정 규칙(p95 ≤1,000)은 표본이 더 쌓여야 적용한다. **앞선 확인**: 단순 예열로 재현되지 않았다. `0.1.31` 배포 직후(검색 요청 0건인 새 파드)에 `POST /hub/search` 를 서로 다른 질문 셋으로 보내니 **0.58 / 0.58 / 0.62s**(외부 왕복 포함), 같은 질문 반복은 **0.05s**(`retrieval_cache`). 1순위는 셋 다 정답 조항(2.12·2.6·4.4). 서버는 요청별 지연을 로그에 안 남기고 그 파드도 이미 교체돼 SYN-010 의 3,300 은 **원인 미해결**로 남긴다. 임베딩은 켜 둔다. 남은 것: 미디에이터가 `retrieval_ms` 를 `/ws` 로 중계하지 않아(류준 확인) 다음 합성 통화에서도 구간을 못 가른다 — `w7-latency-budget` 수집 경로에 얹는다. 원문 — — 모델 탑재 뒤 첫 합성 통화 SYN-010 의 추천 2건 `internal_latency_ms` 3,300 · 2,118(상한). 바로 뒤 SYN-008 8건은 394~778. 첫 요청 예열로 보이지만 가설이다 — 서버 로그의 `retrieval_ms` 로 가른다. 예열이 맞으면 기동 때 더미 질의 한 번이 해법 후보(판정 규칙 124 ⑥ 「p95 1,000 넘으면 임베딩 끔」을 첫 요청 하나로 적용하지 않도록)
