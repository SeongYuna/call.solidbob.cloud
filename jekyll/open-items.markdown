...
- [ ] **C-5 글자 누출 — 「다나카 유이」의 「이」가 확정 자막·DB 에 남는다 (절대 규칙) — 류준** — SYN-017#12 `*** *이`. 규칙·NER 둘 다. 하네스 C-5 누락 0 은 골든셋에 이 형태가 없어서다 → 골든셋에 이름 끝 글자 경계 케이스 추가 + 규칙 보강. E2E 판정기는 이제 글자 단위로 ❌ 를 낸다(`w6-replay-last-turn`)
